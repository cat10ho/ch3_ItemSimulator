...
/*아이템 장착 API*/
router.put("/characters/:characterId/equipped", 
    authMiddleware,validateCharacter,
    validateCharacterInventory,
    validateCharacterItemSlot,
    validateItem,
    validateItemOwnership, 
    ...
    try {
      ...
      const addAbilitie = await prisma.addAbilities.findFirst({ where: { itemId: item.itemId } });
      const Stat = await prisma.stats.findFirst({ where: { characterId: character.characterId } });
...
/*아이템 탈착 API*/
router.put("/characters/:characterId/detachment", 
  authMiddleware,validateCharacter,
  validateCharacterInventory,
  validateCharacterItemSlot,
  validateItem, 
