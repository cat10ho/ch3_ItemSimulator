...
const signUpSchema = Joi.object({loginId: Joi.string().pattern(/^[a-z0-9]+$/).min(4).max(20).required().messages({
...
router.post('/sign-up', async (req, res, next) => {
  try {
    const { loginId, password, verifyPassword, name } = await signUpSchema.validateAsync(req.body, { abortEarly: false });
